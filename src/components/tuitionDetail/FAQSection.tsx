    ...
    <div className="mt-5 bg-white rounded-2xl shadow-lg shadow-slate-200/70 border border-slate-100 p-6 sm:p-8">
      ...
      <div className="space-y-2.5">
        {faqs.map((faq, i) => {
          ...
          return (
            <div key={i} className={`rounded-xl border transition-all duration-200 ${isOpen ? "border-violet-200 bg-violet-50/30 shadow-sm" : "border-slate-100 hover:border-slate-200"}`}>
              <button
                onClick={() => setOpenIndex(isOpen ? null : i)}
                className="flex items-center justify-between w-full px-5 py-4 text-left group"
              >
                <span className={`text-sm font-semibold pr-4 transition-colors ${isOpen ? "text-violet-800" : "text-slate-800"}`}>{faq.question}</span>
                <span
